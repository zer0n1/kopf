...
def guess_resource(settings: configuration.OperatorSettings) -> Optional[references.Selector]:
...
async def touch_command(
    ...
    selector = guess_resource(settings=settings)
    resource = insights.backbone.get(selector) if selector else None
    if resource is None:
        raise click.UsageError(f"Cannot find the peering resource {selector}.")
